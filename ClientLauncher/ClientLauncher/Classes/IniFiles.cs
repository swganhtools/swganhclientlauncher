...
namespace ClientLauncher
{
    ...
    public class IniFiles
    {
        ...
        public string[] GetSectionNames()
        {
            ...
            try
            {
                ...
                len = NativeMethods.GetPrivateProfileSectionNames(ptr,
                    IniFiles.MaxSectionSize, m_path);
        ...
        #region Duplicate Key Helper

        public void AddDuplicateKey(string strSectionName, string strKeyName, string strKeyValue)
        {
            //get the section in question
            List<KeyValuePair<string, string>> lstSection = GetSectionValuesAsList(strSectionName);

            if (!lstSection.Any(kvp => kvp.Key.Equals(strKeyName, StringComparison.InvariantCultureIgnoreCase) && kvp.Value.Equals(strKeyValue, StringComparison.InvariantCultureIgnoreCase)))
            {
                bool inSection = false;

                //open the file up
                TextReader fileIn = new StreamReader(m_path);

                //make a stringbuilder to hold the contents
                StringBuilder sbFileText = new StringBuilder();

                //and read it in, one line at a time
                string strFileLine = fileIn.ReadLine();
                while (strFileLine != null)
                {
                    //append the content
                    sbFileText.AppendLine(strFileLine);

                    //is this the section?
                    if (strFileLine.IndexOf("[" + strSectionName + "]", StringComparison.InvariantCultureIgnoreCase) > -1)
                    {
                        inSection = true;
                    }

                    if ((inSection) && (strFileLine.IndexOf(strKeyName + "=", StringComparison.InvariantCultureIgnoreCase) > -1))
                    {
                        //append the extra line
                        sbFileText.AppendLine("\t" + strKeyName + "=" + strKeyValue);
                    }

                    //and read the next line
                    strFileLine = fileIn.ReadLine();
                }

                //close the readable version
                fileIn.Close();

                //now we make a writable one
                FileStream outFile = new FileStream(m_path, FileMode.Create);
                TextWriter fileOut = new StreamWriter(outFile, Encoding.ASCII);
                fileOut.Write(sbFileText.ToString());
                fileOut.Flush();
                fileOut.Close();
            }
        }
