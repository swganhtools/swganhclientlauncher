...
public static class HtmlToXamlConverter
{
    ...
    {
        //  Implement images
        XmlElement xamlListElement = xamlParentElement.OwnerDocument.CreateElement(null, Xaml_Image, _xamlNamespace);

        if (htmlElement.Attributes["src"] != null)
        {
            xamlListElement.SetAttribute("Source", htmlElement.Attributes["src"].Value);
        }

        if (htmlElement.Attributes["width"] != null)
        {
            xamlListElement.SetAttribute("Width", htmlElement.Attributes["width"].Value.Replace("px", ""));
        }

        if (htmlElement.Attributes["height"] != null)
        {
            xamlListElement.SetAttribute("Height", htmlElement.Attributes["width"].Value.Replace("px", ""));
        }
